    ...
    for row in data:
        ...
        for key in row.keys():
            if key not in ['ID', 'Type', 'Coordinates']:
                value = row[key]
                if key == 'U' and isinstance(value, list):
                    # Split U into u1, u2, u3
                    expanded_row['u1'] = value[0] if len(value) > 0 else 0.0
                    expanded_row['u2'] = value[1] if len(value) > 1 else 0.0
                    expanded_row['u3'] = value[2] if len(value) > 2 else 0.0
                elif key == 'S':
                    # Use MISES instead of S if available
                    if 'MISES' in row:
                        expanded_row['S'] = row['MISES']
                    elif value is None:
                        expanded_row['S'] = 'N/A'
                    elif isinstance(value, list):
                        expanded_row['S'] = ';'.join([str(v) for v in value])
                    else:
                        expanded_row['S'] = str(value)
                else:
                    if value is None:
...
def save_to_txt(data, headers, output_path):
    ...
    with open(output_path, 'wb') as f:
        ...
        # 处理U的分解
        if 'U' in additional_fields:
            additional_fields.remove('U')
            additional_fields.extend(['u1', 'u2', 'u3'])
            ...
            for key in field_order:
                if key in row:
                    ...
                    if isinstance(val, list):
                        ...
                        values.append(str(val))
                elif key in ['u1', 'u2', 'u3'] and 'U' in row:
                    # Handle U decomposition
                    u_value = row['U']
                    if isinstance(u_value, list):
                        if key == 'u1':
                            values.append(str(u_value[0] if len(u_value) > 0 else 0.0))
                        elif key == 'u2':
                            values.append(str(u_value[1] if len(u_value) > 1 else 0.0))
                        elif key == 'u3':
                            values.append(str(u_value[2] if len(u_value) > 2 else 0.0))
                    else:
                        values.append('N/A')
                elif key == 'S' and 'MISES' in row:
                    # Use MISES instead of S
                    values.append(str(row['MISES']))
                else:
                    values.append('N/A')
            line = "\t".join(values)
            f.write((line + "\n").encode('utf-8'))
...
def save_to_excel(data, headers, output_path):
    ...
    try:
        ...
        # Write header
        headers_row = ['ID', 'Type', 'X', 'Y', 'Z']
        for h in headers:
            if h not in ['ID', 'Type']:
                if h == 'U':
                    # Add u1, u2, u3 instead of U
                    headers_row.extend(['u1', 'u2', 'u3'])
                else:
        ...
        for row in data:
            ...
            for key in headers:
                if key in row and key not in ['ID', 'Type', 'Coordinates']:
                    val = row[key]
                    if key == 'U' and isinstance(val, list):
                        # Split U into u1, u2, u3
                        data_row.append(val[0] if len(val) > 0 else 0.0)
                        data_row.append(val[1] if len(val) > 1 else 0.0)
                        data_row.append(val[2] if len(val) > 2 else 0.0)
                    elif key == 'S':
                        # Use MISES instead of S if available
                        if 'MISES' in row:
                            data_row.append(row['MISES'])
                        elif val is None:
                            data_row.append('N/A')
